...
namespace School_Management.API.Services
{
    public class AttendanceService : IAttendanceService
    {
        ...
        public AttendanceService(ApplicationDbContext context, ITeacherRepository teacherRepository)
        {
        ...
        public async Task<int> AttendanceCheck(AttendanceRequest request, Guid userId)
        {
            ...
            var homeRoomId = await context.ClassYear.Where(x => x.Id == request.ClassYearId)
                                                    .Select(g => g.HomeRoomId)
                                                    .FirstOrDefaultAsync();
            if (teacherId != homeRoomId) throw new ForbiddenException("Bạn không phải giáo viên chủ nhiệm của lớp học");
