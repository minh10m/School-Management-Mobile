...
namespace School_Management.API.Services
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<PagedResponse<RoleInfoResponse>> GetAllRoles(RoleFilterRequest request)
        {
            ...
            if(!string.IsNullOrWhiteSpace(request.SortBy))
            {
                ...
                {
                    query = Convert.ToBoolean(request.IsAscending) ? query.OrderBy(x => x.Name) : query.OrderByDescending(x => x.Name);
                }
            }
