...
namespace School_Management.API.Controllers
{
    ...
    public class FeesController : ControllerBase
    {
        ...
        [Route("details")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetAllFeeDetailOfFee([FromQuery] FeeDetailFilterRequest request)
        {
            if (request.PageNumber <= 0) request.PageNumber = 1;
            if (request.PageSize <= 0) request.PageSize = 10;
            var result = await feeService.GetAllFeeDetailOfFee(request);
            return Ok(new
            {
                success = true,
                data = result
        ...
        [Route("{feeId}")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> UpdateFee([FromBody] UpdateFeeRequest request, [FromRoute] Guid feeId)
        {
            var result = await feeService.UpdateFee(request, feeId);
            return Ok(new
            {
                success = true,
