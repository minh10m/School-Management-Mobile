...
export default function AdminStudentDetailScreen() {
  ...
  if (!student) return (
    <SafeAreaView className="flex-1 bg-white items-center justify-center">
      <Text style={{ fontFamily: 'Poppins-Medium' }} className="text-gray-400">Student not found</Text>
    </SafeAreaView>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="flex-row items-center px-6 py-4 bg-white border-b border-gray-100">
        ...
        <Text style={{ fontFamily: 'Poppins-Bold' }} className="text-black text-lg flex-1">Student Detail</Text>
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white items-center py-8 border-b border-gray-100">
           <View className="w-24 h-24 rounded-full bg-blue-100 items-center justify-center mb-4 border-4 border-white shadow-sm">
              <Text style={{ fontFamily: 'Poppins-Bold', color: '#136ADA', fontSize: 36 }}>{student.fullName.charAt(0)}</Text>
           </View>
           <Text style={{ fontFamily: 'Poppins-Bold' }} className="text-black text-xl">{student.fullName}</Text>
           <View className="flex-row items-center gap-2 mt-1">
              <View className="bg-teal-50 px-3 py-1 rounded-full">
                <Text style={{ fontFamily: 'Poppins-SemiBold', fontSize: 10, color: '#0D9488' }}>
        ...
        {isEditing ? (
          <View className="p-6">
            ...
            <EditField label="Birthday (YYYY-MM-DD)" value={form.birthday} onChangeText={(t: string) => setForm({...form, birthday: t})} placeholder="2005-10-10" />
          ...
          <View className="mt-4">
             ...
             <InfoRow label="Phone" value={student.phoneNumber} icon="call-outline" />
             <InfoRow label="Address" value={student.address} icon="location-outline" />
             <InfoRow label="Birthday" value={student.birthday ? new Date(student.birthday).toLocaleDateString('en-GB') : 'N/A'} icon="calendar-outline" />
             <InfoRow label="School Year" value={student.classYearSub?.[0]?.schoolYear?.toString() || '2025-2026'} icon="time-outline" />
             <InfoRow label="User ID" value={student.userId} icon="finger-print-outline" />
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
