    ...
    >
      ...
      <View className="mx-6 mt-6 mb-4 bg-[#136ADA] rounded-[32px] p-6 shadow-xl shadow-blue-200">
        <View className="flex-row justify-between mb-6">
          <View>
            <Text style={{ fontFamily: 'Poppins-Medium' }} className="text-white/70 text-[10px] uppercase tracking-widest">Đơn giá</Text>
            <Text style={{ fontFamily: 'Poppins-Bold' }} className="text-white text-2xl mt-1">{formatCurrency(fee?.amount || 0)}</Text>
          </View>
          <View className="items-end">
            <Text style={{ fontFamily: 'Poppins-Medium' }} className="text-white/70 text-[10px] uppercase tracking-widest">Lớp học</Text>
            <Text style={{ fontFamily: 'Poppins-Bold' }} className="text-white text-lg mt-1">{fee?.className}</Text>
          </View>
        </View>
        
        <View className="mb-2 flex-row justify-between items-end">
          <Text style={{ fontFamily: 'Poppins-Bold' }} className="text-white text-xs">
            Tiến độ: <Text className="text-white/100">{paidCount}/{details.length}</Text>
          </Text>
        ...
        <View className="flex-row items-center gap-2 mt-5">
            ...
            <Text style={{ fontFamily: 'Poppins-Medium' }} className="text-white/70 text-[11px]">Hạn nộp: {fee?.dueDate ? new Date(fee.dueDate).toLocaleDateString('vi-VN') : '---'}</Text>
        </View>
      </View>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View className="flex-row justify-between items-center mb-4 px-1">
             <Text style={{ fontFamily: 'Poppins-Bold', fontSize: 13 }} className="text-[#64748B] uppercase tracking-wider">Danh sách học sinh</Text>
             <View className="bg-gray-100 px-3 py-1 rounded-full">
                <Text style={{ fontFamily: 'Poppins-Bold', fontSize: 10, color: '#94A3B8' }}>{details.length} HỌC SINH</Text>
             </View>
          </View>
        }
        renderItem={({ item }) => {
          ...
          return (
            <View className="bg-white rounded-[24px] px-5 py-4 border border-gray-100 flex-row items-center shadow-sm">
              <View className="w-12 h-12 rounded-full bg-blue-50 items-center justify-center border border-blue-100">
                <Text style={{ fontFamily: 'Poppins-Bold', color: '#136ADA', fontSize: 16 }}>{item.studentName.charAt(0).toUpperCase()}</Text>
              </View>
              <View className="flex-1 ml-4 mr-2">
                <Text style={{ fontFamily: 'Poppins-Bold', fontSize: 15 }} className="text-[#1E293B] mb-0.5">{item.studentName}</Text>
                <Text style={{ fontFamily: 'Poppins-Medium', fontSize: 11 }} className="text-[#64748B]">{formatCurrency(item.amountDue)}</Text>
              </View>
              
              {isPaid ? (
                <View className="bg-green-50 px-3 py-1.5 rounded-xl flex-row items-center gap-1.5 border border-green-100">
                  <Ionicons name="checkmark-circle" size={14} color="#16A34A" />
                  <Text style={{ fontFamily: 'Poppins-Bold', color: '#16A34A', fontSize: 10 }}>ĐÃ ĐÓNG</Text>
                </View>
              ) : (
                <TouchableOpacity
                  activeOpacity={0.7}
                  className="bg-red-50 px-3 py-1.5 rounded-xl flex-row items-center gap-1.5 border border-red-100"
                  onPress={() => Alert.alert('Thông báo', `Học sinh ${item.studentName} chưa hoàn thành học phí.`)}
                >
                  <Ionicons name="time-outline" size={14} color="#EF4444" />
                  <Text style={{ fontFamily: 'Poppins-Bold', color: '#EF4444', fontSize: 10 }}>CHƯA ĐÓNG</Text>
                </TouchableOpacity>
              )}
            </View>
          );
        ...
        ListEmptyComponent={
          <View className="items-center py-20">
            ...
            <Text style={{ fontFamily: "Poppins-Medium" }} className="text-gray-400 mt-4 text-center px-10">Không tìm thấy học sinh nào phù hợp.</Text>
          </View>
        }
      />
    </AdminPageWrapper>
