...
export default function LessonDetailScreen() {
  ...
  // Menu state for deleting items (format: 'video-{id}' or 'assignment-{id}')
  ...
  const fetchLessonDetails = useCallback(async () => {
    ...
    try {
      ...
      const [lessonData, videosResponse, assignmentsData] = await Promise.all([
        ...
        lessonVideoService
          .getLessonVideos({ lessonId: id, pageNumber: 1, pageSize: 100 })
          .catch(() => ({ items: [], totalCount: 0 })),
        lessonAssignmentService
          .getLessonAssignments({ lessonId: id, pageNumber: 1, pageSize: 100 })
          .catch(() => ({ items: [], totalCount: 0 })),
      ]);
  ...
  const handleCreateAssignment = async () => {
    ...
    try {
      ...
      const fileAsset =
        selectedFile && !selectedFile.canceled ? selectedFile.assets[0] : null;
  ...
  const handleDeleteAssignment = async (assignmentId: string) => {
    Alert.alert("Xóa bài tập", "Bạn có chắc chắn muốn xóa bài tập này không?", [
      { text: "Hủy", style: "cancel" },
      {
        text: "Xóa",
        style: "destructive",
          ...
          >
            ...
            <View className="px-6 mb-8">
              <View className="flex-row justify-between items-center mb-5">
                ...
                <TouchableOpacity
                  onPress={() => router.push({ pathname: '/teacher/lessons/add-video', params: { lessonId: id } })}
                    ...
                    >
                      ...
                      >
                        <Ionicons
                          ...
                          size={24}
                          color={
                            activeVideoUrl === video.url ? "#136ADA" : "#9ca3af"
                          }
                        />
                      </View>
            ...
            <View className="px-6 mb-8">
              <View className="flex-row justify-between items-center mb-5">
                ...
                <TouchableOpacity
                  onPress={() => router.push({ pathname: '/teacher/lessons/add-assignment', params: { lessonId: id } })}
                ...
                assignments.map((assignment) => (
                  ...
                  >
                    <TouchableOpacity
                      onPress={() => {
                        router.push(
                          `/teacher/lesson-assignments/${assignment.id}` as any,
                        );
                      }}
                      activeOpacity={0.7}
                    ...
                    >
                      ...
                      <View className="flex-1 pr-2">
                        <Text
                          style={{ fontFamily: "Poppins-Bold" }}
                          className="text-gray-800 text-sm mb-0.5"
                          numberOfLines={1}
                        >
                          {assignment.title}
                        </Text>
                        ...
                        <View className="flex-row items-center mt-0.5">
                          <Ionicons name="document-attach-outline" size={12} color="#9CA3AF" />
                          <Text
                            style={{ fontFamily: "Poppins-Medium" }}
                            className="text-gray-400 text-[10px] ml-1 flex-1"
                            numberOfLines={1}
                          >
                    ...
                    <View className="relative z-50">
                      <TouchableOpacity
                        onPress={(e) => {
                          e.stopPropagation();
                          setOpenMenuId(
                            openMenuId === `assignment-${assignment.id}`
                              ? null
